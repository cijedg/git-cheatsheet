...
```mermaid
flowchart LR
    HEAD -->|ссылается на| branch[Ветка]
    branch -->|содержит| commit[Последний коммит]
```

## 🔍 Как устроен HEAD
1. **Физически**: Файл в `.git/HEAD`
2. **Содержимое**:
   ```bash
   $ cat .git/HEAD
   ref: refs/heads/main  # или master в старых репозиториях
   ```
...
### 3. Отмена последних изменений
```bash
git reset --hard HEAD~1
```

## 📌 Важные особенности
- **Динамический указатель**: Всегда показывает на:
  - Последний коммит в текущей ветке (обычный режим)
  - Конкретный коммит при detached HEAD (режим просмотра истории)

- **Автообновление**: При новых коммитах:
  1. Обновляется файл ветки (refs/heads/main)
  2. HEAD продолжает указывать на ту же ветку

## 🛠️ Полезные команды
| Команда | Описание |
|---------|----------|
| `git symbolic-ref HEAD` | Показать текущую ветку |
| `git rev-parse HEAD` | Показать хеш коммита |
| `git log -1 HEAD` | Показать последний коммит |

## ⚠️ Режим detached HEAD
```mermaid
flowchart LR
    HEAD -->|прямо указывает на| old[Старый коммит]
    style old stroke:#f66
```

Когда возникает:
```bash
git checkout e007f50  # Переход к конкретному коммиту
```
Что делать:
- Создать новую ветку: `git switch -c new-branch`
- Вернуться: `git switch main`

---

> 💡 **Совет**: Используйте `HEAD` вместо хеша в командах — это безопаснее и читаемее!
> Пример: `git diff HEAD~3 HEAD` сравнит изменения за 3 коммита.

## 🔄 Жизненный цикл HEAD
1. Делаем коммит → Git обновляет:
   ```bash
   .git/refs/heads/main  # Новый хеш
   .git/HEAD             # Остается ref: refs/heads/main
   ```
2. Переключаем ветку → Git меняет:
   ```bash
   .git/HEAD             # Теперь ref: refs/heads/feature
   ```
``` 
